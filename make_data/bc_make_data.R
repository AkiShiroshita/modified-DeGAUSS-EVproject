...
writeVector(grid, "bc_monthly/grid.gpkg", overwrite=TRUE)

# Create monthly BC files -------------------------------------------------

grid <- vect("bc_monthly/grid.gpkg")
...
filenames <- expand.grid(year = 2000:2017, month = sprintf("%02d", 1:12)) %>%
  mutate(
    ...
    input_filename = paste0("bc_monthly_raw/Monthly/V5NA05.02.HybridBC-BC.NorthAmerica.", year, "-", year, "-", month_abbr, ".nc"),
    output_filename = paste0("bc_monthly/", year, month, ".fst")
  ) 
