...
# 1. Defining exposure period: birth date to birth date + 1 year (first year of life)
...
# Exposure period = first year of life (birth to birth + 1 year)
# start_date = TN_DOB (date of birth)
# end_date = TN_DOB + 1 year - 1 day (exactly 1 year)
d[, `:=`(
...
d_tn_out <- d[TN_flag == 0 | is.na(TN_flag)] 
