# ============================================================================
# NO2 Exposure Assessment - Pregnancy/Dyad Cohort (Monthly)
# ============================================================================
# This script assigns NO2 (nitrogen dioxide) exposure estimates for the
# dyad cohort during pregnancy period using monthly aggregated NO2 data.
...
d <- d[state == "TN" & start_date >= as.Date("2001-01-01") & end_date <= as.Date("2016-12-31")]
...
d_grid <- sf::st_read('data/no2_schwartz_monthly/grid_no2.gpkg') |> st_transform(crs = 5072)
...
d_no2_summary <- d[, rbindlist(no2), by = recip][, .(average_no2_pregnancy = mean(value, na.rm = TRUE)), by = recip]
